...
func normalizeMap(expectedSymbol string, item map[string]any) (Tick, bool) {
	...
	symbol := strings.ToUpper(firstString(item, "symbol", "s", "secSymbol", "code", "ticker", "sym"))
	if symbol == "" {
	...
	last := firstFloat(item, "last", "lastPrice", "price", "matchPrice", "close", "c", "lp", "mp", "p", "lastMatchedPrice")
	...
	return Tick{
		...
		Ask:         ask,
		Last:        last,
		Volume:      firstInt64(item, "volume", "vol", "matchVolume", "lastQuantity", "quantity", "q", "tradeQuantity", "tq", "lastVolume"),
		TimestampMS: ts,
	}, true
}
